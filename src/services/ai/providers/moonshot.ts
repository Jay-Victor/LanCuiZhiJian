...
export class MoonshotProvider implements AIProvider {
  ...
  async processText<T = unknown>(
    ...
    if (optimizer.isCircuitOpen('moonshot')) {
    ...
    const metric = optimizer.startRequest('moonshot', model, options.task)
    
    const promptHash = optimizer.hashString(systemPrompt)
    const cacheKey = optimizer.getCacheKey('moonshot', model, options.task, text, promptHash, maxTokens, temperature)
    ...
    const deduplicationKey = optimizer.getDeduplicationKey('moonshot', model, options.task, text, promptHash, maxTokens, temperature)
      ...
      const result = await optimizer.executeWithDeduplication(
        ...
        () => optimizer.executeWithConcurrency(
          () => optimizer.executeWithRetry(
            async () => {
              const taskTimeout = optimizer.getTaskTimeout(options.task, 'moonshot')
              ...
              if (!response.ok) {
                ...
                throw new Error(`Moonshot API 错误 (${response.status}): ${errorMessage}`)
              }
