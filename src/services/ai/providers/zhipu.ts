...
export class ZhipuProvider implements AIProvider {
  ...
  async processText<T = unknown>(
    ...
    if (optimizer.isCircuitOpen('zhipu')) {
    ...
    const metric = optimizer.startRequest('zhipu', model, options.task)
    
    const promptHash = optimizer.hashString(systemPrompt)
    const cacheKey = optimizer.getCacheKey('zhipu', model, options.task, text, promptHash, maxTokens, temperature)
    ...
    const deduplicationKey = optimizer.getDeduplicationKey('zhipu', model, options.task, text, promptHash, maxTokens, temperature)
      ...
      const result = await optimizer.executeWithDeduplication(
        ...
        () => optimizer.executeWithConcurrency(
          () => optimizer.executeWithRetry(
            async () => {
              const taskTimeout = optimizer.getTaskTimeout(options.task, 'zhipu')
              ...
              if (!response.ok) {
                ...
                throw new Error(`智谱 API 错误 (${response.status}): ${errorMessage}`)
              }
